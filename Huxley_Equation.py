...
def f(t, y):
    ...
    X = y[0] #u1
    Y = y[1] #u2
    return [Y, -c*Y+X+(X**3)-2*(X**2)]

# Training Points  t is \xi
t = np.linspace(-2, 7, 100).reshape(-1, 1)

# Initial value (boundary -2)
y0_list = [0.19557, 0.111244]
...
print(nn)
nn.reset_weights()
nn.train(maxiter=3000, iprint=200)   # Maximum number of iterations
...
t_test = np.linspace(-2, 7.5, 150).reshape(-1,1)
